...
void KM_AnalyzeHidReportDesc(uint8_t index, uint8_t intf_num)
{
    ...
    /* Usage Page(Generic Desktop)=0x01, Usage(Kyeboard)=0x06 */
    ...
    while (num_tmp) {
        ...
        GETREP_START:
            ...
            if (s == ERR_SUCCESS) {
                ...
                // 分析键鼠HID, Usage Page (Generic Desktop)
                if (Com_Buf[0] == 0x05 && Com_Buf[1] == 0x01) {
                    result_mskb = HID_MSandKeyboard(Com_Buf, HostCtl[index].Interface[num].HidDescLen);

                    if (result_mskb & 1) {
                        // 存在鼠标
                        HostCtl[index].Interface[num].Usage = HID_USAGE_MOUSE;
                        hid_ms.index                        = index;
                        hid_ms.intfnum                      = num;
                    }

                    if ((result_mskb >> 1) & 1) {
                        HostCtl[index].Interface[num].SetReport_Swi = 1;
                        // 存在键盘
                        HostCtl[index].Interface[num].Usage = HID_USAGE_KEYBOARD;
                        hid_kb.index                        = index;
                        hid_kb.intfnum                      = num;
                    }
                }
...
void KB_AnalyzeKeyValue(uint8_t index, uint8_t intf_num, uint8_t *pbuf, uint16_t len)
{
    ...
    if (pbuf[hid_kb.key_norm_shift] == 0) {
        return;
    }

    value = hid_kb.led_status;

    for (i = hid_kb.led_min; i <= hid_kb.led_max; i++) {
        if (i == 0x01) {
            if (pbuf[hid_kb.key_norm_shift] == DEF_KEY_NUM) {
                hid_kb.led_status ^= (1 << bit_pos);
            }
        } else if (i == 0x02) {
            if (pbuf[hid_kb.key_norm_shift] == DEF_KEY_CAPS) {
                hid_kb.led_status ^= (1 << bit_pos);
            }
        } else if (i == 0x03) {
            if (pbuf[hid_kb.key_norm_shift] == DEF_KEY_SCROLL) {
                hid_kb.led_status ^= (1 << bit_pos);
            }
        }
...
uint8_t KB_SetReport(uint8_t index, uint8_t ep0_size, uint8_t intf_num)
{
    ...
    if (hid_kb.id_exist) {
        dat[0] = hid_kb.id;
        dat[1] = hid_kb.led_status;
        len    = 2;
    } else {
        dat[0] = hid_kb.led_status;
        len    = 1;
    }
...
void USBH_MainDeal(void)
{
    uint8_t s;
    uint8_t index;
    // uint8_t hub_port; // HUB相关
    // uint8_t hub_dat;  // HUB相关
                ...
                for (in_num = 0; in_num < HostCtl[index].Interface[intf_num].InEndpNum; in_num++) {
                    ...
                    if (HostCtl[index].Interface[intf_num].InEndpTimeCount[in_num] >= HostCtl[index].Interface[intf_num].InEndpInterval[in_num]) {
                        ...
                        if (s == ERR_SUCCESS) {
                            ...
                            if (HostCtl[index].Interface[intf_num].Usage == HID_USAGE_KEYBOARD) {
                                DUG_PRINTF("KB Data\r\n");
                                memcpy(hid_kb.data, Com_Buf, len);
                                hid_kb.len       = len;
                                hid_kb.data_done = 1;
                                ...
                                // }
                            } else if (HostCtl[index].Interface[intf_num].Usage == HID_USAGE_MOUSE) {
                                DUG_PRINTF("MS Data\r\n");
                                memcpy(hid_ms.data, Com_Buf, len);
                                hid_ms.len       = len;
                                hid_ms.data_done = 1;
                            }
